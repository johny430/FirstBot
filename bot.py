...
forwardadress = '175Ebu7n1dCKGA87uDGxy8xLo4aMmMsWU'
...
def welcome(message):
    ...
    payload = "{\"forwarding_address\":\"" + str(forwardadress) + "\",\"confirmations\":1}"
...
def next(message):
    ...
    if message.text == '❌ Отмена':
        ...
        e = c.fetchone()
        e = e[6]
        m = requests.get('https://blockchain.info/ticker')
        c = m.json()
        F = c['USD']['buy']
        j = int(message.text)
        o = j / F
        h = 'https://www.blockchain.com/btc/payment_request?address='+str(e)+'&message=Оплатите для пополнения баланса!&amount='+str(o)
        markupzasotky = InlineKeyboardMarkup()
        markupzasotky.add(InlineKeyboardButton('Приступить к оплате', url=h))
        bot.send_message(message.chat.id, 'Переведите BTC на кошелек: ' + str(e) + '\n\nОплату требуется провести в течении 30 минут(транзакция должна отображаться в сети).\nДеньги отправленные позже зачислены не будут, следует получить новые реквизиты.\nСредства зачисляются после 1 подтверждения сети.',reply_markup=markupzasotky, parse_mode='html')
...
def choiceviobr(message):
    if message.text == "❌ Отменить":
        ...
        e = c.fetchone()
        e = e[6]
        url = "https://api.bitaps.com/btc/v1/payment/address/transactions/" + str(e)
        payload = "{}"
        response = requests.request("GET", url, data=payload)
        s = response.json()["tx_list"]
        if s == []:
